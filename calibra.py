...
# Incluye selector T4 (Dinámico/Fijo), Ciec opcional, gráficos y descargas.
...
st.caption("Hojas mínimas: **ensayos** (ensayo_id, loss_obs_pct, MAX_PLANTS_CAP, fecha_siembra, pc_ini, pc_fin[, Ca, Cs]) "
...
def non_overlapping_states(emer_daily: pd.Series) -> pd.DataFrame:
    ...
    v = emer_daily.to_numpy(float); n = len(v)
...
with st.spinner("Calculando densidad efectiva x (estados normalizada) por ensayo…"):
    ...
    for _, row in ens.iterrows():
        ...
        Ca_row = int(row.get("Ca", 250) if not pd.isna(row.get("Ca", np.nan)) else 250)
        Cs_row = int(row.get("Cs", 250) if not pd.isna(row.get("Cs", np.nan)) else 250)
        ...
        if use_ciec:
            Ciec = ciec_series(
                s.index, f_sow,
                "Cobertura (%)" if mode_canopy=="Cobertura (%)" else "LAI dinámico",
                int(t_lag), int(t_close), float(cov_max), float(lai_max), float(k_beer),
                int(Ca_row if Ca_row>0 else 1), int(Cs_row if Cs_row>0 else 1), float(LAIhc)
            )
            # compute_canopy espera 'mode_canopy' interno: unificar etiqueta
            # Normalizamos el modo para compute_canopy:
            mode_norm = "Cobertura (%)"
        else:
            mode_norm = "Cobertura (%)"  # no se usa si no hay Ciec

        if use_ciec:
            # Recalcular con el modo interno correcto
            Ciec = ciec_series(
                s.index, f_sow,
                "Cobertura (%)" if mode_canopy=="Cobertura (%)" else "LAI dinámico",
                int(t_lag), int(t_close), float(cov_max), float(lai_max), float(k_beer),
        ...
        if t4_mode.startswith("Fijo"):
            ...
            def effective_T4_days_local(sow_date, pc_ini_, pc_fin_):
                start_S4 = max(pc_ini_, sow_date + pd.Timedelta(days=60))
                if start_S4 > pc_fin_: return 1
                return max(1, (pc_fin_ - start_S4).days + 1)
            T4 = effective_T4_days_local(f_sow, pc_ini, pc_fin)
        ...
        xs.append(float(auc(s.index[mask_pc], D[mask_pc])))
...
st.caption(f"Modo T4: **{t4_mode}** · Ciec: **{'Sí' if use_ciec else 'No'}**")
    ...
    with c2:
        ...
        with pd.ExcelWriter(bio, engine="xlsxwriter") as writer:
            ...
            pd.DataFrame({
                ...
                "T4_mode":[out["T4_mode"].iloc[0]], "use_ciec":[use_ciec],

...
# Representación semanal: plantas·m²·semana⁻¹
# ===============================================================
...
st.title("🌱 PREDWEEM — Calibrado Encadenado (S₁→S₂→S₃→S₄ · pl·m²·sem⁻¹)")
...
P12 = 1/10   # S₁→S₂ (~10 días)
P23 = 1/15   # S₂→S₃ (~15 días)
P34 = 1/20   # S₃→S₄ (~20 días)

# ---------- FUNCIÓN DE PÉRDIDA ----------
def loss_fun(x):
    x = np.asarray(x, dtype=float)
    raw = ALPHA * x / (1.0 + (ALPHA * x / LMAX))
    return np.minimum(raw, x)  # la pérdida nunca supera la densidad efectiva
...
# propagación encadenada con controles
for t in range(1, n):
...
df_states["EFF"] = (W_S["S1"]*S1 + W_S["S2"]*S2 + W_S["S3"]*S3 + W_S["S4"]*S4)

# reagrupa a escala semanal (sumatoria de los 7 días)
df_week = (df_states
    .resample("W", on="fecha")
    .sum()
    .reset_index())

# ---------- GRÁFICO PRINCIPAL (plantas·m²·semana⁻¹) ----------
st.subheader("📊 Dinámica semanal (S₁–S₄)")
fig = go.Figure()
fig.add_trace(go.Bar(x=df_week["fecha"], y=df_week["S1"], name="S₁ Emergencia"))
fig.add_trace(go.Bar(x=df_week["fecha"], y=df_week["S2"], name="S₂ Joven"))
fig.add_trace(go.Bar(x=df_week["fecha"], y=df_week["S3"], name="S₃ Intermedia"))
fig.add_trace(go.Bar(x=df_week["fecha"], y=df_week["S4"], name="S₄ Adulta"))
...
# ---------- PÉRDIDA ----------
densidad_efectiva = df_states["EFF"].sum()
...
st.download_button("💾 Descargar resultados CSV (semanal)", csv, "calibrado_encadenado_semana.csv", "text/csv")

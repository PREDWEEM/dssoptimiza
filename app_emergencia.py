...
import io, json, math, datetime as dt, itertools, random
import numpy as np
import pandas as pd
...
# ---------- CONFIGURACIÓN ----------
st.set_page_config(page_title="🌾 PREDWEEM — Cohortes + PCC + Optimización", layout="wide")
...
def auc_time(time_series, values, mask=None):
    if mask is not None:
        time_series = time_series[mask]
        values = np.array(values)[mask]
    if len(time_series) < 2:
        return float(np.nansum(values))
    days = (time_series - time_series.min()).days
    return float(np.trapz(values, days))

def cap_cumulative(vals, cap, mask=None):
    if mask is None:
        mask = np.ones_like(vals, dtype=bool)
    cum = np.cumsum(np.where(mask, vals, 0.0))
    return np.minimum(cum, cap)

# ---------- UI DE PARÁMETROS ----------
with st.sidebar:
    st.header("Configuración general")
    sow_date = pd.to_datetime(
        st.date_input(
            "Fecha de siembra",
            value=ts.min().date() + dt.timedelta(days=30),
            min_value=ts.min().date(),
            max_value=ts.max().date(),
        )
    )

    st.markdown("### Parámetros de crecimiento del cultivo")
    t_lag = st.number_input("t_lag (días)", 0, 100, 20, 1)
    t_close = st.number_input("t_close (días)", 0, 150, 80, 1)
    lai_max = st.number_input("LAI_max", 0.0, 10.0, 6.0, 0.1)
    cov_max = st.number_input("Cobertura máxima (%)", 0.0, 100.0, 95.0, 1.0)
    k_beer = st.number_input("Coef. Beer-Lambert (k)", 0.1, 2.0, 0.65, 0.01)

    st.markdown("### Pesos por estado (S1–S4)")
    FC_S = {
        "S1": st.number_input("FC_S1", 0.0, 3.0, W_S["S1"], 0.001),
        "S2": st.number_input("FC_S2", 0.0, 3.0, W_S["S2"], 0.001),
        "S3": st.number_input("FC_S3", 0.0, 3.0, W_S["S3"], 0.001),
        "S4": st.number_input("FC_S4", 0.0, 3.0, W_S["S4"], 0.001),
    }

# ---------- CÁLCULO DEL CULTIVO ----------
days_since = np.array([(d - sow_date).days for d in ts])
def logistic_between(days, start, end, y_max):
    t_mid = 0.5 * (start + end)
    r = 4.0 / max(1.0, (end - start))
    return np.clip(y_max / (1.0 + np.exp(-r * (days - t_mid))), 0.0, y_max)

fc_dyn = np.where(days_since < t_lag, 0.0, logistic_between(days_since, t_lag, t_close, cov_max / 100))
...
# ===============================================================
# 🌾 PERIODO CRÍTICO DE COMPETENCIA (PCC) — definición manual por fechas
# ===============================================================
with st.sidebar:
    ...
    # --- Fechas por defecto ---
    default_ini = pd.Timestamp(year=ts.min().year, month=10, day=10)
    default_fin = pd.Timestamp(year=ts.min().year, month=11, day=4)
    ...
    if usar_pcc:
        duracion_pcc = (pcc_fin_date - pcc_ini_date).days
        st.caption(
            f"PCC activo: del **{pcc_ini_date.date()}** al **{pcc_fin_date.date()}** "
            f"({duracion_pcc} días)"
        )
    else:
        st.caption("El modelo integrará sobre **todo el ciclo de cultivo**.")

    mask_pcc = (ts >= pcc_ini_date) & (ts <= pcc_fin_date)

# ===============================================================
# 🌱 SIMULACIÓN DE COHORTES SECUENCIALES
# ===============================================================
np.random.seed(42)
emerrel = np.clip(np.sin(np.linspace(0, np.pi, len(ts))) + 0.05 * np.random.randn(len(ts)), 0, None)
auc_cruda = auc_time(ts, emerrel)
factor_area_to_plants = A2_CAP_DEFAULT / auc_cruda if auc_cruda > 0 else None

mask_since = (ts >= sow_date)
S1 = np.where(mask_since, emerrel, 0.0)
S2 = np.roll(S1, 10); S2[:10] = 0
S3 = np.roll(S2, 15); S3[:25] = 0
S4 = np.roll(S3, 20); S4[:45] = 0

S1, S2, S3, S4 = [np.clip(s, 0, None) for s in (S1, S2, S3, S4)]
S_total = S1 + S2 + S3 + S4

st.write("Bloque 1 listo — con PCC manual por fechas.")

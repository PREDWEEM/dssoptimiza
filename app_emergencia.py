# -*- coding: utf-8 -*-
# app_emergencia.py — PREDWEEM · Supresión (EMERREL × (1−Ciec)) + Control (AUC) + Fenología por COHORTES (S1..S4)
# - Sin ICIC
...
APP_TITLE = "PREDWEEM · Supresión (1−Ciec) + Control (AUC) + Cohortes · A2 seleccionable · Optimización"
...
births = np.where(mask_since_sow.to_numpy(), births, 0.0)  # anular antes de siembra
...
if pre_selR:   apply_efficiency_per_state(weights_residual(pre_selR_date,  pre_res_dias),   ef_pre_selR,  ["S1","S2","S3","S4"])  # forzado
...
if factor_area_to_plants is None or not np.isfinite(factor_area_to_plants):
    ...
    if not mspc.any():
        st.info("No hay días dentro del Periodo Crítico en el rango de datos cargado.")
    else:
        ...
        st.download_button("Descargar composición porcentual en PC (CSV)", df_pc_pct.to_csv(index=False).encode("utf-8"),
...
# Panel de parámetros de optimización
with st.sidebar:
    ...
    # Rango de siembra usando mismos límites
...
# Generación de lista de duraciones (incluye max si paso no cae justo)
...
# Guardamos settings actuales como defaults para la búsqueda
...
def recompute_for_sow(sow_d: dt.date):
    ...
    def roll_sum_shift_local(s: pd.Series, win: int, shift_days: int) -> pd.Series:
        return s.rolling(window=win, min_periods=0).sum().shift(shift_days)

    S1_coh = roll_sum_shift_local(births_s,  6,  1).fillna(0.0)
    S2_coh = roll_sum_shift_local(births_s, 21,  7).fillna(0.0)
    S3_coh = roll_sum_shift_local(births_s, 32, 28).fillna(0.0)
    ...
    FC_S_local = {"S1": 0.0, "S2": 0.3, "S3": 0.6, "S4": 1.0}
    S1_pl = np.where(mask_since, S1_arr * one_minus_Ciec_loc * FC_S_local["S1"] * factor_area, 0.0)
    S2_pl = np.where(mask_since, S2_arr * one_minus_Ciec_loc * FC_S_local["S2"] * factor_area, 0.0)
    S3_pl = np.where(mask_since, S3_arr * one_minus_Ciec_loc * FC_S_local["S3"] * factor_area, 0.0)
    S4_pl = np.where(mask_since, S4_arr * one_minus_Ciec_loc * FC_S_local["S4"] * factor_area, 0.0)
...
def evaluate(sd: dt.date, schedule: list):
    ...
    for a in schedule:
        if a["kind"] in ("pre_selR","post_selR"):
            ...
            w = mask.astype(float)
            apply(w, a["eff"], a["states"])  # pre_selR ya viene con S1..S4
...
if results:
    ...
    # Pintar bandas del mejor en el Gráfico 1 existente

...
# ===============================================================
# 🧩 BLOQUE 7 — OPTIMIZACIÓN (Grid · Random · Recocido Simulado)
# ===============================================================

# --- Completar el cap y reescalado si no quedó en el bloque 6 ---
if factor_area_to_plants is not None:
    eps = 1e-12
...
# ------------------ AGREGACIÓN SEMANAL (vista rápida previa) ------------------
...
# ------------------ A2 por AUC ------------------
if factor_area_to_plants is not None:
    auc_cruda_posi = max(auc_cruda, 0.0)
    ...
    auc_sup      = auc_time(ts, sup_equiv,  mask=(ts.dt.date >= sow_date))
    auc_sup_ctrl = auc_time(ts, supc_equiv, mask=(ts.dt.date >= sow_date))
    A2_sup_final  = min(MAX_PLANTS_CAP, MAX_PLANTS_CAP * (auc_sup      / max(auc_cruda_posi, 1e-12)))
    A2_ctrl_final = min(MAX_PLANTS_CAP, MAX_PLANTS_CAP * (auc_sup_ctrl / max(auc_cruda_posi, 1e-12)))
else:
    A2_sup_final = A2_ctrl_final = float("nan")

# ------------------ x y pérdida ------------------
if factor_area_to_plants is not None:
    X2 = float(np.nansum(plantas_supresion_cap[(ts.dt.date >= sow_date).to_numpy()]))
    X3 = float(np.nansum(plantas_supresion_ctrl_cap[(ts.dt.date >= sow_date).to_numpy()]))
else:
    X2 = X3 = float("nan")

# ------------------ Panel previo (opcional) ------------------
st.subheader(f"📊 Vista previa: EMERREL + aportes (cap A2={int(MAX_PLANTS_CAP)}) — Serie semanal (W-MON)")
fig_prev = go.Figure()
fig_prev.add_trace(go.Scatter(x=ts, y=df_plot["EMERREL"], mode="lines", name="EMERREL (cruda)"))
fig_prev.add_trace(go.Scatter(x=sem_x, y=plm2sem_sin_ctrl_cap, name="Aporte semanal (sin control, cap)", yaxis="y2", mode="lines+markers"))
fig_prev.add_trace(go.Scatter(x=sem_x, y=plm2sem_con_ctrl_cap, name="Aporte semanal (con control, cap)", yaxis="y2", mode="lines+markers", line=dict(dash="dot")))
...
st.markdown(
    ...
    f"**A2_sup (cap):** **{A2_sup_final if np.isfinite(A2_sup_final) else float('nan'):.1f}** pl·m²  \n"
    f"**A2_ctrl (cap):** **{A2_ctrl_final if np.isfinite(A2_ctrl_final) else float('nan'):.1f}** pl·m²"
)

# =====================================================
# 🧠 OPTIMIZACIÓN — UI + helpers + ejecución
# =====================================================
st.markdown("---")
st.header("🧠 Optimización")

# -------- Parámetros de búsqueda (sidebar) --------
with st.sidebar:
    ...
    preR_min_back  = st.number_input("PresiembraR: hasta X días antes de siembra", 14, 120, 14, 1)
...
def recompute_for_sow(sow_d: dt.date, T12_: int, T23_: int, T34_: int):
    ...
    for i in range(len(births)):
        if i - int(T12_) >= 0:
            moved = births[i - int(T12_)]; S1[i - int(T12_)] -= moved; S2[i] += moved
        if i - (int(T12_) + int(T23_)) >= 0:
            moved = births[i - (int(T12_)+int(T23_))]; S2[i - (int(T12_)+int(T23_))] -= moved; S3[i] += moved
        if i - (int(T12_) + int(T23_) + int(T34_)) >= 0:
            moved = births[i - (int(T12_)+int(T23_)+int(T34_))]; S3[i - (int(T12_)+int(T23_)+int(T34_))] -= moved; S4[i] += moved
...
def evaluate(sd: dt.date, schedule: list):
    ...
    # Reglas duras de fecha (+14d postR para graminicida)
    for a in schedule:
        ...
        if a["kind"] == "post_gram":
            ...
            if postR_dates and min((d - pr).days for pr in postR_dates) < 14:
                return None
    ...
    A2_sup  = min(MAX_PLANTS_CAP, MAX_PLANTS_CAP*(auc_sup/ max(auc_cruda_loc,1e-12)))
    A2_ctrl = min(MAX_PLANTS_CAP, MAX_PLANTS_CAP*(auc_sup_ctrl/ max(auc_cruda_loc,1e-12)))

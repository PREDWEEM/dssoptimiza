...
if results:
    ...
    if envb is None:
        ...
        fig_best1.update_layout(
            ...
            xaxis_title="Tiempo", yaxis_title="EMERREL",
            yaxis2=dict(overlaying="y", side="right", title="pl·m²·sem⁻¹", position=1.0, range=[0, 100], tick0=0, dtick=20, showgrid=False)
        )
        fig_best1.add_trace(go.Scatter(x=df_week_b["fecha"], y=df_week_b["pl_sin_ctrl_cap"],
                                       name="Aporte semanal (sin control, cap) — mejor", yaxis="y2", mode="lines+markers"))
        fig_best1.add_trace(go.Scatter(x=df_week_b["fecha"], y=df_week_b["pl_con_ctrl_cap"],
                                       name="Aporte semanal (con control, cap) — mejor", yaxis="y2", mode="lines+markers", line=dict(dash="dot")))

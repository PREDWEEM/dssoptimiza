...
def evaluate(sd: dt.date, schedule: list):
   ...
   # -------- Reaplicar agenda (con jerarquía y gateo) — FINAL CORREGIDO
if "envb" in locals() and envb is not None:
    fechas_eval = envb.get("fechas_d", envb.get("ts", None))
    if fechas_eval is None:
        st.warning("No se encontraron fechas válidas en envb."); fechas_eval = pd.Series(dtype="datetime64[ns]")
    ...
    st.warning("No se encontró entorno 'envb'; omitiendo reaplicación de agenda.")
